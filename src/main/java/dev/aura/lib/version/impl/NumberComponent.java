...
public class NumberComponent extends VersionComponent {
  ...
  @SuppressFBWarnings(
    ...
    justification =
        "Implementation requires the inversion of the value. The deeper implementations will never return Integer.MIN_VALUE in any realistic case"
  )
  @Override
  public int compareTo(VersionComponent that) {
    VersionComponent.Type thatType = that.getVersionComponentType();

    if (thatType == VersionComponent.Type.LIST) return -that.compareTo(this);
    else if (thatType == VersionComponent.Type.STRING) return 1;
